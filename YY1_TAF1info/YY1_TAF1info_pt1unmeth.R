...
for (tissue_TFBS in unique_tissues) {
  ...
  com_res <- commonPeakResult(commonPeaks = com, return_common_peak_sites = TRUE,
                                  ...
                                  return_summary = TRUE, meth_level = "unmethylated")
  
  com_peak_list <- com_res$common_peak_list
  com_peaks <- com_peak_list[[1]]
  write_tsv(com_peaks, file = paste0(tissue_TFBS, "_YY1_TAF1_unmeth_com_peaks", ".tsv"))
  
  com_peaks_func <- greatAnnotate(peaks = com_peaks, return_annotation = TRUE)
  write_tsv(com_peaks_func, file = paste0(tissue_TFBS, "_YY1_TAF1_unmeth_com_GO", ".tsv"))
  ...
  write_tsv(genes, file = paste0(tissue_TFBS, "_YY1_TAF1_unmeth_com_genes_list", ".tsv"))
  ...
  write_tsv(YY1_exclu_peaks, file = paste0(peak_id_01, "_unmeth_exclu_peaks", ".tsv"))
  
  YY1_exclu_peaks_func <- greatAnnotate(peaks = YY1_exclu_peaks, return_annotation = TRUE)
  write_tsv(YY1_exclu_peaks_func, file = paste0(peak_id_01, "_unmeth_exclu_GO", ".tsv"))
  
  # make a copy of the data && use rGREAT to get associated genes
  YY1_exclu_peaks2 <- as.data.frame(YY1_exclu_peaks[,1:3], stringsAsFactors = FALSE)
  jobx <- submitGreatJob(YY1_exclu_peaks2, species = "hg38")
  
  # Get associated genes
  gene_table <- getRegionGeneAssociations(jobx)
  gene_table1 <- gene_table$annotated_genes
  genes <- unique(as.data.frame(gene_table1@unlistData))
  write_tsv(genes, file = paste0(peak_id_01, "_unmeth_exclu_genes_list", ".tsv"))
  ...
  write_tsv(TAF1_exclu_peaks, file = paste0(peak_id_02, "_unmeth_exclu_peaks", ".tsv"))
  
  TAF1_exclu_peaks_func <- greatAnnotate(peaks = TAF1_exclu_peaks, return_annotation = TRUE)
  write_tsv(TAF1_exclu_peaks_func, file = paste0(peak_id_02, "_unmeth_exclu_GO", ".tsv"))
  
  # make a copy of the data && use rGREAT to get associated genes
  TAF1_exclu_peaks2 <- as.data.frame(YY1_exclu_peaks[,1:3], stringsAsFactors = FALSE)
  jobx <- submitGreatJob(TAF1_exclu_peaks2, species = "hg38")
  
  # Get associated genes
  gene_table <- getRegionGeneAssociations(jobx)
  gene_table1 <- gene_table$annotated_genes
  genes <- unique(as.data.frame(gene_table1@unlistData))
  write_tsv(genes, file = paste0(peak_id_02, "_unmeth_exclu_genes_list", ".tsv"))
}
